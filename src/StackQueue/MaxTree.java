...
/**
 * @author Azreal 对于一个没有重复元素的整数数组，请用其中元素构造一棵MaxTree，
 *         MaxTree定义为一棵二叉树，其中的节点与数组元素一一对应，同时对于MaxTree的每棵子树，
 *         它的根的元素值为子树的最大值。现有一建树方法，对于数组中的每个元素，其在树中的父亲为数
 *         组中它左边比它大的第一个数和右边比它大的第一个数中更小的一个。若两边都不存在比它大的数，
 *         那么它就是树根。请设计O(n)的算法实现这个方法。 给定一个无重复元素的数组A和它的大小n，
...
public class MaxTree {
	...
	public static void main(String[] args) {
		...
		int[] A = { 3, 1, 4, 2 };
		int[] B = maxTree.buildMaxTree(A, 4);
		for (int i : B) {
